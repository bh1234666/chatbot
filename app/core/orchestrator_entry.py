...
def _workspace_file_url(req, filename: str, workspace_dir: str = "") -> str:
    url = f"/v1/chat/files/{req.archive_id}/{req.group_id}/{filename}"
    if workspace_dir:
        try:
            from app.llm.tools import workspace as _ws_tool

            token = _ws_tool.workspace_token(workspace_dir)
            if token:
                return f"{url}?workspace_token={token}"
        except Exception:
            pass
    return url
...
def _autofix_deliverables(
    ...
    *,
    ...
    """Apply conservative late deliverable candidates to the current plan."""
    if plan is None or not workspace_dir:
        return
    try:
        ...
        candidates = _collect_deliverable_candidates(
            ...
            files_before=files_before or set(),
        )
    except Exception:
        try:
            log.exception("round2 deliverable autofix failed")
        except Exception:
            pass
        return
    if not candidates:
    ...
    seen = {item.replace("\\", "/") for item in current}
    added: list[str] = []
    for candidate in candidates:
        ...
        added.append(item)
    if added:
        plan.deliverables = current
        try:
            debug.log(
                "round2.deliverables.autofix.applied",
                f"added {len(added)} deliverable candidate(s): {added[:8]}",
                {"added": added[:12]},
            )
        except Exception:
            pass
...
def _classify_interrupt_payloads(payloads: list[dict] | None) -> tuple[list[dict], list[dict], list[dict]]:
    """Split queued interrupt payloads by source semantics."""
    user_payloads: list[dict] = []
    background_payloads: list[dict] = []
    other_payloads: list[dict] = []
    for item in payloads or []:
        payload = dict(item or {})
        kind = str(payload.get("kind") or "user").strip().lower()
        source = str(payload.get("source") or "").strip().lower()
        if kind == "background" or source.startswith("env_background"):
            background_payloads.append(payload)
        elif kind == "user":
            user_payloads.append(payload)
        else:
            other_payloads.append(payload)
    return user_payloads, background_payloads, other_payloads


def _background_interrupt_context_block(payloads: list[dict]) -> str:
    """Build a compact task-local note for background task updates."""
    lines = [
        "## Background Task Update",
        "A non-user background task reported progress while you were working.",
        "This is not a stop request. Continue the current task unless these facts require a small adjustment.",
        "Background task outputs stay on disk; read the referenced files only when they become relevant.",
        "",
    ]
    for payload in payloads[:4]:
        meta = payload.get("meta") or {}
        task_id = str(meta.get("task_id") or payload.get("task_id") or "background").strip()
        reminder_kind = str(meta.get("reminder_kind") or payload.get("reminder_kind") or "update").strip()
        message = str(payload.get("message") or "").strip()
        status = str(meta.get("status") or payload.get("status") or "").strip()
        result_path = str(
            meta.get("result_abs_path") or payload.get("result_abs_path") or meta.get("result_path") or payload.get("result_path") or ""
        ).strip()
        status_path = str(
            meta.get("status_abs_path") or payload.get("status_abs_path") or meta.get("status_path") or payload.get("status_path") or ""
        ).strip()
        detail = [f"task_id={task_id}", f"kind={reminder_kind}"]
        if status:
            detail.append(f"status={status}")
        if result_path:
            detail.append(f"result_path={result_path}")
        if status_path:
            detail.append(f"status_path={status_path}")
        lines.append(f"- {'; '.join(detail)}")
        if message:
            lines.append(f"  reminder: {message[:240]}")
    lines.append("")
    lines.append("If you need the result, use read_file on the absolute result_path/status_path instead of expecting inline output.")
    return "\n".join(lines)
...
async def orchestrate(
    ...
    *,
    ...
    interrupt_payloads_getter=None,
    ...
    get_group_guard().set_stage(req.archive_id, req.group_id, req.user_id, "round1")
    if is_trivial:
    ...
    round1_interrupt_payloads: list[dict] = []
    if abort_event.is_set() and interrupt_payloads_getter is not None:
        try:
            round1_interrupt_payloads = [
                dict(item) for item in (interrupt_payloads_getter() or [])
            ]
        except Exception as _e_r1_intr:
            debug.log(
                "round1.interrupt_payloads.error",
                f"failed to collect interrupt payloads: {type(_e_r1_intr).__name__}: {_e_r1_intr}",
            )
            round1_interrupt_payloads = []
        _r1_user_payloads, _r1_background_payloads, _r1_other_payloads = _classify_interrupt_payloads(
            round1_interrupt_payloads
        )
        if _r1_user_payloads:
            merged_message = _message_with_user_interrupts(req.message, round1_interrupt_payloads)
            if merged_message != req.message:
                req = req.model_copy(update={"message": merged_message})
                debug.log(
                    "round1.interrupt_rerun",
                    f"merged {len(_r1_user_payloads)} user interrupt payload(s); rerunning round1",
                    {"messages": [p.get("message") for p in _r1_user_payloads[:3]]},
                )
                abort_event.clear()
                ...
                r1 = await _round1(req.user_name, req.message, hot_user)
        elif _r1_background_payloads and not _r1_other_payloads:
            debug.log(
                "round1.background_interrupt_deferred",
                f"deferred {len(_r1_background_payloads)} background interrupt payload(s) to round2",
            )
    # From this point on Round1 routing is complete. Any new interrupt should use
    # Round2 semantics, including the pre-tool context/safety-gate preparation
    # below. Snapshot immediately so interrupts in that gap are not swallowed.
    ...
    _background_reminder_intent = _has_background_reminder_intent(req.message)
    ...
    try:
        ...
        if _is_environment_mode():
            ...
            if _background_reminder_intent and not _tool_concept_intent:
                _env_project_tool_intent = True
                if not _env_project_route_reason:
                    _env_project_route_reason = "background reminder intent"
    except Exception:
        pass
    ...
    _base_needs_recent_context = (
        # 最近上下文质量门控: 覆盖"刚才/前面/群里/大家/谁说/讨论/最近"等跟进语义。
        bool(needs_recall)
        or _has_implicit_recall_intent(req.message)
        or _context_followup_intent
        or _constraint_review_followup_intent
    ...
    if not needs_tools and _background_reminder_intent and _env_project_tool_intent and (not _tool_concept_intent):
        complexity = "medium" if complexity == "easy" else complexity
        needs_tools = True
        debug.log(
            "round1.safety_gate",
            "easy→medium + needs_tools=True (environment background reminder intent)",
        )
    ...
    try:
        ...
        if complexity == "easy":
            ...
            background_interrupt_resume_count = 0
            background_interrupt_contexts: list[str] = []
            guard.set_stage(req.archive_id, req.group_id, req.user_id, "round2")
            if _stop_mode_triggered:
                base_msgs.append({
                    "role": "system",
            ...
            while current_stage is not None:
                ...
                for _bg_context in background_interrupt_contexts[-2:]:
                ...
                background_interrupt_contexts.clear()
                ...
                queued_interrupt_payloads: list[dict] = []
                if interrupt_payloads_getter is not None:
                    try:
                        queued_interrupt_payloads = [
                            dict(item) for item in (interrupt_payloads_getter() or [])
                        ]
                    except Exception as _e_intr_payloads:
                        debug.log(
                            "round2_5.interrupt_payloads.error",
                            f"failed to collect interrupt payloads: {type(_e_intr_payloads).__name__}: {_e_intr_payloads}",
                        )
                        queued_interrupt_payloads = []
                _user_interrupt_payloads, _background_interrupt_payloads, _other_interrupt_payloads = _classify_interrupt_payloads(
                    queued_interrupt_payloads
                )
                _background_only_abort = (
                    abort_event.is_set()
                    and bool(_background_interrupt_payloads)
                    and not _user_interrupt_payloads
                    and not _other_interrupt_payloads
                )
                if _background_only_abort and background_interrupt_resume_count < 3:
                    background_interrupt_resume_count += 1
                    _bg_context = _background_interrupt_context_block(_background_interrupt_payloads)
                    background_interrupt_contexts.append(_bg_context)
                    try:
                        _append_round2_dynamic_user_tail(base_msgs, _bg_context)
                    except Exception as _e_bg_tail:
                        debug.log(
                            "round2_5.background_tail.error",
                            f"failed to append background dynamic guidance: {type(_e_bg_tail).__name__}: {_e_bg_tail}",
                        )
                    abort_event.clear()
                    debug.log(
                        "round2_5.background_resume",
                        f"background-only interrupt at stage={current_stage}; resuming round2 with {len(_background_interrupt_payloads)} payload(s)",
                    )
                    continue
        ...
        if workspace_dir:
            ...
            for fname in ws_tool.list_generated_files(workspace_dir):
                ...
                url = _workspace_file_url(req, fname, workspace_dir)
                ...
                if missing:
                    ...
                    if _resolved:
                        ...
                        for _orig, _actual in _resolved.items():
                            ...
                            _url = _workspace_file_url(req, _actual, workspace_dir)
        ...
        if r2_was_aborted:
            guard.set_stage(req.archive_id, req.group_id, req.user_id, "round2_5")
        ...
        try:
            ...
            if _round2_voice_reply_file:
                ...
                if os.path.isfile(_vp):
                    voice_reply_file = (
                        _vf,
                        _workspace_file_url(req, _vf, workspace_dir),
                        _vp,
                    )
                ...
                if decision.use_voice and decision.voice_text:
                    ...
                    if not speak_text:
                        ...
                        try:
                            ...
                            pass
                        if r.ok and r.paths:
                            voice_url = _workspace_file_url(req, voice_fname, workspace_dir)
