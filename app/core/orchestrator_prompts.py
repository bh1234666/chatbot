...
def _build_round2_system_prompts(
    *,
    ...
    out.append({"role": "system", "content": (
            ...
            "Use base kinds code/read/edit/draw/image_gen/verify/tts plus project-analysis kinds project_map/file_summary/impact_review, but use image_gen only when it is present in the delegate schema; difficulty and retry strength belong in `mode`, not kind. Material reading that writes internal `.txt` evidence remains `read`; executable preparation before reading can be a separate `code` helper. `draw` is for charts from data, while `image_gen` is for AI text-to-image or image-to-image generation.\n"
