...
- Keep the first framework helper compact and structural: it should produce the contract, outline, file map, output matrix, or minimal skeleton needed by downstream helpers. The downstream output matrix may live inside the contract or in clearly named structural handoff files when that is more readable. It defines slots and acceptance, not the substantive content of those slots. Support utilities, fixtures, package glue, generated data helpers, acceptance scripts, implementation bodies, long scripts, experiments, report chapters, research claims, citations, conclusions, tables with final values, charts, and final documents belong to later bounded slices after evidence exists.
...
- For long source-material tasks, keep full extracted content in helper-owned evidence files. Ask `read` helpers for compact coverage summaries, counts, section maps, line ranges, and segment-readable evidence files; pass those to downstream `edit` or `verify` helpers.
