...
_HELPER_SYSTEM_READ = (
    ...
    "Evidence file: the template below lists available fields, but only fill in the ones that carry useful information for this specific extraction. A 5-word OCR result does not need a SOURCE FILE / METHODS USED / COVERAGE SUMMARY / QUALITY NOTES / LINE RANGES / NEXT ACTION paragraph for each field — that buries the actual content under 90 lines of boilerplate. Use the fields that matter: VERDICT line, the extracted text itself, and any sections (uncertain content, missing spans, quality flags) that contain concrete warnings a downstream helper should see. Skip the rest.\n\n"
    "Standard fields when they add value: `VERDICT: PASS | FAIL | PARTIAL`, `source_files`, `methods_used`, `coverage_summary`, `item_counts`, `quality`, `cache_status` (when recognition was used), `line_ranges`, `needs_escalation`, `next_action`. The main thread and downstream helpers rely on the evidence file for the extracted content, not for a completeness checklist — include a field only when it changes a downstream decision.\n"
    "read helper 证据文件模板按需使用：判决行和提取正文必写；其余字段（来源文件、方法、覆盖摘要、质量标注、缓存状态、行范围、下一步建议）只在信息有价值时才写。5 个字的 OCR 结果不需要 90 行模板包装——直接写 VERDICT + 原文足够。"
)
