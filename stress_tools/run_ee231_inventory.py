"""EE231 engineering management inventory short test (6.3).

Asks the orchestrator to enumerate F:\\chatbot\\电子231工程管理 and produce a
classified inventory before any synthesis. Mirrors the IELTS material_index
pattern: coverage first, no full reading. Verifies that the read helper can
classify Office/PDF/image files with a planned read strategy.
"""
...
WALL_CAP_SEC = 1200
STALL_CAP_SEC = 90
SOURCE_DIR = Path(r"F:\chatbot\电子231工程管理")
...
def main() -> int:
    ...
    session_dir = RUN_DIR / f"ee231_inventory_{run_id}"
    work_dir = session_dir / "ee231_workspace"
    if work_dir.exists():
    ...
    body = {
        "user_id": f"ee231-inv-{run_id}",
        "user_name": "LongTest",
        "message": (
            "Stage A only: scan the entire source directory and produce an "
            f"inventory ledger. Source: {SOURCE_DIR}. Do not synthesize a "
            "report yet. Produce only material_index.json (or .md) in the "
            "current working directory listing every file with: relative path, "
            "category (group/student/zip/docx/pdf/image/audio/video/other), "
            "size in bytes, and planned read strategy (text-extract / "
            "office-extract / OCR / archive-expand / skip-with-reason). "
            "Mark duplicates and obviously empty files. After the index is "
            "written, stop and report the index path. "
            "中文概要：仅做覆盖清单——按分组/学生/zip/Office/图片分类，列出大小与读取策略，"
            "先产出 material_index 后停。"
        ),
        "current_dir": str(work_dir),
        "persona_id": "environment",
        "archive_id": f"ee231-inv-{run_id}",
        "client_msg_id": f"ee231-inv-{run_id}",
    }
    ...
    print(f"[ee231-inv] run={run_id}")
    print(f"[ee231-inv] work_dir={work_dir}")
    ...
    def _on_text(text: str, t: float) -> None:
        ...
        if index_seen_at is None and (
            "material_index" in text or "coverage" in text
        ):
            index_seen_at = t
            print(f"[ee231-inv] material_index reference in stream at {index_seen_at:.1f}s")
    ...
    try:
        ...
        if stream_result.truncated_wall_cap:
            print(f"[ee231-inv] wall-cap hit at {stream_result.elapsed_sec:.1f}s")
        elif stream_result.truncated_stall_cap:
            print(
                f"[ee231-inv] stall-cap hit at {stream_result.elapsed_sec:.1f}s "
                f"(no SSE activity for {STALL_CAP_SEC}s)"
            )
        elif stream_result.stream_error:
            print(f"[ee231-inv] stream error: {stream_result.stream_error}")
        else:
            print(f"[ee231-inv] stream ended at {stream_result.elapsed_sec:.1f}s")
    finally:
        sse_fh.close()
    ...
    if not declared and capture.archive_id:
        print(f"[ee231-inv] no LLM declaration of {EXPECTED_ARTIFACT}; asking LLM follow-up")
        ...
        if followup.get("ok"):
            print(f"[ee231-inv] follow-up answer: {followup.get('answer_text','')[:280]}")
    ...
    print(f"[ee231-inv] summary written to {summary}")
